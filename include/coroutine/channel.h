...
template <typename T = std::monostate> class RecvAwaiter
{
  ...
  public:
    ...
    auto await_suspend(std::coroutine_handle<Coroutine::promise_type> handle) noexcept
    {
...
template <typename T> class Channel
{
  ...
  private:
    bool send_impl(std::coroutine_handle<Coroutine::promise_type> handle, SendAwaiter<T>* awaiter);
    bool recv_impl(std::coroutine_handle<Coroutine::promise_type> handle, RecvAwaiter<T>* awaiter);
    bool is_closed() const { return is_closed_; }
    bool is_empty() const { return resource_.empty() && send_requests_.empty(); }
    bool is_full() const { return resource_.size() >= capacity_ && recv_request_.empty(); }
    struct SendRequest
    {
...
template <typename T>
bool Channel<T>::send_impl(std::coroutine_handle<Coroutine::promise_type> handle, SendAwaiter<T>* send_awaiter)
{
    Coroutine notify_coro;
    {
        ...
        {
            // channel未关闭且已满，阻塞
            send_requests_.push({send_awaiter, handle});
            return true;
        }
        // channel未关闭且不满，唤醒
        ...
        {
            auto& recv_request = recv_request_.front();
            notify_coro = std::move(recv_request.coro);
            recv_request.awaiter->set_value(std::move(resource_.front()));
            recv_request_.pop();
            resource_.pop();
        }
    }
...
template <typename T>
bool Channel<T>::recv_impl(std::coroutine_handle<Coroutine::promise_type> handle, RecvAwaiter<T>* recv_awaiter)
{
    Coroutine notify_coro;
    {
        std::lock_guard lock(mutex_);
        ...
        if (is_empty())
        {
            recv_request_.push({recv_awaiter, handle});
            return true;
        }
        // channel不空，唤醒
        // 保持FIFO
        ...
        {
            auto& [send_awaiter, send_coro] = send_requests_.front();
            notify_coro = std::move(send_coro);
            resource_.push(send_awaiter->get_value());
            send_requests_.pop();
        }
